...
# Configuração do log em português
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(BASE_DIR / "pdad_app.log", encoding="utf-8", mode="a"),
...
class App(tk.Tk):
    ...
    def load_data_task(self) -> None:
        try:
            ...
            logging.error(f"Erro crítico: {exc}")
            error_msg = str(exc)
    ...
    def _on_data_loaded(self, df: pd.DataFrame, ras_ordenadas: List[str], ranking: List[Tuple[str, float]], t_mor: int, t_dom: int) -> None:
        ...
        logging.info("Interface atualizada com RAs carregadas.")
    ...
    def update_plots(self) -> None:
        ...
        if "id_genero" in df_filt.columns:
            ...
            if df2 is not None and not df2.empty:
                ...
                x = range(len(g1))
                self.ax1.bar(x, g1.values, color="royalblue")
                self.ax1.set_xticks(list(x))
        ...
        s1, s2, _ = self.engine.obter_estatisticas(df_filt, codigo_ra)
        ...
        if 'escolaridade' in df_filt.columns:
            ...
            for codigo, quantidade in escolaridade_counts.items():
                try:
                    ...
                    nome_nivel = niveis.get(codigo_int, f"Nível {codigo}")
        ...
        if 'escolaridade' in df_filt.columns:
            ...
            if not escolaridade_valida.empty:
                ...
                if not escolaridade_contagens.empty:
                    ...
                    niveis = {1: "Sem instrução", 2: "Fundamental incompleto", 3: "Fundamental completo", 
                              4: "Médio incompleto", 5: "Médio completo", 6: "Superior incompleto", 
                              7: "Superior completo", 8: "Sem classificação"}
                    escolaridade_mais_comum = niveis.get(codigo_top, f"Nível {codigo_top}")
...
"""
        ...
        if file_path:
            with open(file_path, 'w', encoding='utf-8') as f:
                f.write(relatorio)
            logging.info(f"Relatório exportado: {file_path}")
